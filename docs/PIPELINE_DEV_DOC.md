...
| 数据库 | PostgreSQL 17 | 统一库名 `doubao`，业务表加 `qa_` 前缀 |
...
|------|------|
| 单条 Query (API) | 40~50s |
| 单条 Query (Web UI) | 60~120s（含等待回答生成） |
| 10 条批量 (API) | ~30min（含 180s 间隔） |
| 10 条批量 (Web) | ~20min（含 60s 间隔） |
...
| 场景 | 耗时 |
|------|------|
| 单条链接（8080 API 正常） | 2~5s |
| 单条链接（通用网页） | 1~3s |
| 单条链接（404 重试 3 次） | ~14s（2+4+8s 退避） |
| 10 条链接混合批量 | 20~60s |
| LLM 使用 | **否** |
...
| 场景 | 耗时 |
|------|------|
| 单条抖音视频（下载+提音+转写） | 60~120s |
| `--audio-concurrency 2` 两条并行 | 90~180s（取决于视频时长） |
| 命中无视频/不可转写 | 2~10s（快速标记 `skip/error`） |
...
| 单条结构化（含 LLM 后处理） | 3~8s |
| 10 条含 LLM 后处理 | 30~80s（串行） |
| 10 条纯规则（关闭 LLM） | <1s |
...
> 基于 2 条 Query（Q0001, Q0002）的真实跑批数据

| 步骤 | 耗时 | 占比 | 瓶颈分析 |
|------|------|------|---------|
| Step 1: Collect | 98.84s | 35% | LLM 回答生成 + 深度思考搜索 |
| Step 2: Crawl | 40.79s | 14% | 抖音 API 重试 + 404 链接退避 |
| Step 2.5: Enrich | 0.42s | <1% | 纯 DB 操作，极快 |
| Step 3: Structure | 142.08s | 50% | **串行 LLM 调用是最大瓶颈** |
| **总计** | **282.8s** | 100% | |

### 10.1 最新环节耗时估算（2026-03-17，小批 run-sync）

> 基于最近小批验证（旧单 + 新单混合）的观测区间，适用于当前默认配置（`crawl-concurrency=3`，`audio-concurrency=1~2`，`structure-concurrency=5`）。

| 环节 | 估算耗时 | 备注 |
|------|---------|------|
| Collect（Web） | 50~90s / query | 主要受回答生成和页面稳定等待影响 |
| Crawl | 2~5s / link（混合批） | 23 链接批次实测约 68s，抖音/通用混合 |
| Enrich-Douyin | <100ms / link | 纯 DB merge，通常不是瓶颈 |
| Audio-Transcribe | 60~120s / 抖音视频 | 视频时长越长耗时越高；失败项通常 2~10s 快速返回 |
| Structure | 3~8s / link（含 LLM） | 可通过 `--link-ids` 做小批复盘 |
...
| C | Structure 阶段并发 | 当前串行调 LLM，10 条需 30~80s | 并发 3~5 路可提速 3x |
...
- **Structure 并发化**：将 Step 3 的 LLM 调用改为 `asyncio.gather` 并发 3~5 路，预计从 142s 降至 30~50s
...
| 服务 | 启动方式 | 默认端口 |
|------|---------|---------|
| PostgreSQL | `brew services start postgresql@17` | 5432 |
